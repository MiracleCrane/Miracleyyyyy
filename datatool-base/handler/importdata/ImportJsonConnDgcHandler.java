/*
 * 文 件 名:  ImportJsonConnDgcHandler.java
...
/**
 * 该处理器负责导入来自dgc平台json类型的数据连接文件资产
 *
 * @author z00569896
...
public class ImportJsonConnDgcHandler extends AbstractImportHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(ImportJsonConnDgcHandler.class);
    ...
    public void handleImportTask(TaskModel taskModel) {
        ...
        List<DtConnectionEntity> dtConnectionEntities = convertConnFormat(taskModel);
        for (DtConnectionEntity dtConnectionEntity : dtConnectionEntities) {
            try {
                ...
                LOGGER.error("Import json conn fail.", e);
    ...
    private List<DtConnectionEntity> convertConnFormat(TaskModel taskModel) {
        List<DtConnectionEntity> dtConnectionEntities = new ArrayList<>();
        try {
            JSONArray jsonArray = JSONObject.parseObject(taskModel.getFileContent()).getJSONArray("contentDTOS")
                    .getJSONObject(0).getJSONArray("uploadContent");
            for (int i = 0; i < jsonArray.size(); i++) {
                JSONObject jsonObject = jsonArray.getJSONObject(i);
                JSONObject dwConfig = jsonObject.getJSONObject("dwConfig");
                DtConnectionEntity dtConnectionEntity = new DtConnectionEntity();
                dtConnectionEntity.setName(jsonObject.getString("dwName"));
                dtConnectionEntity.setType(CommonUtils.getDgcDBType(jsonObject.getString("dwType")));
                dtConnectionEntity.setUser(dwConfig.getString("dbUserName"));
                dtConnectionEntity.setHost("");
                dtConnectionEntity.setPort(Integer.parseInt(dwConfig.getString("port")));
                dtConnectionEntities.add(dtConnectionEntity);
            }
            taskModel.setTotal(dtConnectionEntities.size());
            taskModel.setSingleFile(false);
            // 更新history表total字段
            importHelper.updateImportHistoryEntity(taskModel.getTaskId(), dtConnectionEntities.size());
        } catch (DataToolRuntimeException e) {
            LOGGER.error("Convert dgc json conn to object format fail.", e);
            errorHandle(taskModel, e);
        } catch (Exception e) {
            LOGGER.error("Convert dgc json conn to object format fail.", e);
            errorHandle(taskModel, ExceptionCode.DATATOOL_FILE_CONTENT_CONVERT_FAIL);
        }
        return dtConnectionEntities;
    }
}